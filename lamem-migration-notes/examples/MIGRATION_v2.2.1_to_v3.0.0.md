# Migrating LaMEM from v2.2.1 to v3.0.0
...
1. **Rewrite your solver block.** The old top-level `SolverType`, `DirectSolver`,
   ...
   `<SolverOptionsStart>…<SolverOptionsEnd>` block. **→ [§4](#4-solver-options-the-headline-change)**
   ⚠️ These removed parameters are **silently ignored** — your model will still run,
   but with v3.0.0 *default* solver settings, not yours. See the pitfall in [§8](#8-pitfalls--troubleshooting).
2. **2D setups: change `nel_y = 1` → `nel_y = 2`.** Fewer than 2 cells in any
   direction is now a hard error. **→ [§5.1](#51-minimum-2-cells-per-direction-breaking)**
3. **Graded meshes: drop non-unit `bias_*`.** Bias ratios ≠ 1.0 are now rejected. **→ [§5.2](#52-mesh-bias-ratios-deprecated)**
4. **Out-of-plane background shear removed.** `exz_strain_rates` / `eyz_strain_rates`
   (and their `*_time_delims`) no longer exist. **→ [§5.3](#53-background-strain-rates-reduced)**
5. **Gravity-survey feature removed** entirely (`gravity.cpp/h` deleted). **→ [§5.4](#54-gravity-survey-feature-removed)**
6. **Paths changed:** `input_models/` → `examples/`; `matlab/`, `utils/`, top-level
   `docs/` removed; new `info/` reference dir. **→ [§3](#3-repository-layout-changes)**
7. **Tests renamed:** `t1_…` → `t01_…`, `.expected` files lost their `-pN` suffix.
   Update any downstream CI. **→ [§6](#6-tests--examples)**
8. **If you patched the C++ source:** every `CHKERRQ(ierr)` is now `PetscCall(...)`,
   ...
   **→ [§7](#7-for-people-who-patched-the-source)**
...
to `<SolverOptionsStart>` per [§4.3](#43-removed--new-parameter-map).
...
> **Migrating a file?** Work through the [Quick self-check](#quick-self-check-when-migrating-a-file)
> at the top of this guide ([§0](#0-tldr--the-migration-checklist)) — it turns every pitfall
